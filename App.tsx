...
const ensurePermissions = (u: UserAccount): UserAccount => {
  ...
  const currentPerms = typeof u.permissions === 'object' ? u.permissions : {};

  // Merge defaults with current, keeping current values if they exist, but ensuring all keys from defaults are present
  const mergedPermissions = {
    ...(defaults as any),
    ...(currentPerms as any)
  };

  // Ensure 'master' logic is correct based on role even if permission object existed
  (mergedPermissions as any).master = u.role === 'Master' || !!(currentPerms as any).master;

  return { ...u, permissions: mergedPermissions };
};
